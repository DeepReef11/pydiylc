...
![corpus](https://img.shields.io/badge/corpus%20recognition-99.97%25-brightgreen)
...
**Corpus coverage:** 99.97% component recognition on the DIYLC regression corpus (106,569 of 106,599 components across 709 of 711 real community layouts, including legacy v1 `<Layout>` files), with **all 709 readable files round-tripping cleanly with identical component count + type signature** through read → save → re-read. The 2 read misses are physically truncated XML in the upstream corpus; only ~30 components remain unrecognized across an extreme long tail (`ElectrolyticCanCapacitor`, `TriPadBoard`, `FuseSymbol`, `CrystalOscillator`, `NeutrikJack1_4`, `JFETSymbol`, …), each appearing in 2-3 files.
